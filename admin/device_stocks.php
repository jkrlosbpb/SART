        ...
        <div class="container-fluid">
            <div class="row-fluid">
				...
				<div class="span12" id="content">
                     <div class="row-fluid">
					<a href="dashboard.php" class="btn btn-info"  id="return" data-placement="right" title="Click to return" ><i class="icon-arrow-left icon-large"></i> Atrás</a>      <a href="add_Device.php" class="btn btn-info"  id="add" data-placement="right" title="Click para Agregar Dispositivo" ><i class="icon-plus-sign icon-large"></i> Agregar Dispositivo</a>
                 ...
                 ?>	 
				   <div id="block_bg" class="block">
                        <div class="navbar navbar-inner block-header">
                             <div class="muted pull-left"><i class="icon-reorder icon-large"></i> Inventario de Dispositivos </div>
                          <div class="muted pull-right">
								Total Dispositivo(s): <span class="badge badge-info"><?php  echo $count; ?></span>
							 </div>
						  </div>
						  
                 <h4 id="sc">Lista de Inventario de Dispositivos 
 ...
 <br/>
	<div class="pull-left">
			  ...
			  <?php	
	           $count_item=mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			   where dev_name = 'TECLADO' OR dev_name = 'keyboard' OR dev_name = 'TECLADO'
			   OR dev_name = 'kyboard' OR dev_name = 'kboard' OR dev_name = 'kbard' OR dev_name = 'kbord'OR dev_name = 'kbrd' OR dev_name = '(Teclado)'
			   OR dev_name = '(kbrd)'
			   ORDER BY stdevice.id DESC");
	           $count = mysql_num_rows($count_item);
               ?>					
				<li class="">
					<a href="Keyboard.php">Teclado(s)&nbsp;<span class="label label-default"> <?php echo $count;?></span></a>
				</li>
  ...
  <div class="row-fluid"> 
     <div class="empty">
	     <div class="pull-right">
		   <a href="print_all_stock.php" class="btn btn-info" id="print" data-placement="left" title="Click para Imprimir"><i class="icon-print icon-large"></i> Imprimir Lista</a> 		      
		   <script type="text/javascript">
		     $(document).ready(function(){
		     $('#print').tooltip('show');
		     $('#print').tooltip('hide');
...
<?php
		...
		LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id ORDER BY stdevice.id DESC")or die(mysql_error());
		...
		<td><?php
			...
			<td><?php echo $row['cod_garantia']; ?> </td>
			...
			<td>
				<?php
				$d1 = $row['fecha_compra']; 
				$nuevafecha = strtotime ( '+4 Year' , strtotime ( $d1 ) ) ;
			...
			<td><?php
			   ...
			   {
			   echo '<div class="alert alert-info"><i class="icon-ok"></i> '.$row['dev_status'].'</div>';
		       }
			   else if($row['dev_status']=='Reparado')
			   {
			   echo '<div class="alert alert-success"><i class="icon-wrench"></i> <strong>'.$row['dev_status'].'</strong></div>';
		       }
		       else if($row['dev_status']=='Dañado')
			   {
			   echo '<div class="alert alert-warning"><i class="icon-warning-sign"></i> <strong>'.$row['dev_status'].'</strong></div>';
		       }
		       else
			   {
			   echo '<div class="alert alert-danger"><i class="icon-trash"></i> <strong>'.$row['dev_status'].'</strong></div>';
		       };
			  ?></td>
			
			<?php include('toolttip_edit_delete.php'); ?>												
			<td class="empty" width="80"><a rel="tooltip"  title="Editar Dispositivo" id="<?php echo $id; ?>" href="device_edit.php<?php echo '?id='.$id; ?>" class="btn btn-success"><i class="icon-pencil"> Editar</i></a></td>
		</tr>
