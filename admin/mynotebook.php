    ...
    <body>
        ...
        <!-------------------------------block ------------------------------>
            <div id="block_bg" class="block">
				...
				Lista de Notebook: <span class="badge badge-info"><?php echo $count_my_device; ?></span>
				</div>
								
           </div>
							
<!-----------------------------------for Print display visible------------------------------------>	
<h4 id="sc">Ubicación: <?php echo $location_row['stdev_location_name']; ?> Lista de Notebook(s)
...
$my_device = mysql_query("select * from location_details    
				...
				OR NOT EXISTS 
                (select * from location_details where dev_status='Deshechado') 
				and dev_name = 'Notebook'    and stdev_id = '$get_id'")or die(mysql_error());
...
<tr>
		<td><?php
			...
			<a rel="tooltip"  title="Actualizar Estado" id="t<?php echo $id; ?>" href="devlpc_update_status.php<?php echo '?id='.$id; ?>&<?php echo 'stdev_id='.$get_id; ?>" class="btn btn-success"><i class="icon-upload-alt"></i> Estado</a></td>
		</tr>
