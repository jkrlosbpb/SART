           ...
           <div id="" class="muted pull-right">
			...
			$my_device = mysql_query("select * from location_details    
             ...
             (select * from location_details where dev_status='Deshechado')
			  and dev_name = 'TECLADO'  and stdev_id = '$get_id'
			  OR NOT EXISTS 
             (select * from location_details where dev_status='Deshechado')
			  and dev_name = 'TECLADO'    and stdev_id = '$get_id'
			  OR NOT EXISTS 
			...
			TECLADO List: <span class="badge badge-info"><?php echo $count_my_device; ?></span>
			</div>
								
        </div>
							
<!-----------------------------------for Print display visible------------------------------------>	
<h4 id="sc">Location:<?php echo $location_row['stdev_location_name']; ?> TECLADO List
			...
			<?php	
             ...
             (select * from location_details where dev_status='Deshechado')
			  ...
			  ?>
				...
				<li class="active">			
				    <a href= 'myKeyboard.php<?php echo '?stdev_id='.$get_id; ?>')">TECLADO&nbsp;<span class="label label-default"> <?php echo $count;?></span></a>					
				</li>
...
<tr>
		<td><?php
			...
			<td width="185" class="empty"><a rel="tooltip"  title="Transferir Dispositivo" id="e<?php echo $id; ?>" href="transfer.php<?php echo '?id='.$id; ?>&<?php echo 'stdev_id='.$get_id; ?>" class="btn btn-warning"><i class="icon-move"></i> Transferir </a>
			<a rel="tooltip"  title="Actualizar Estado" id="t<?php echo $id; ?>" href="devlk_update_status.php<?php echo '?id='.$id; ?>&<?php echo 'stdev_id='.$get_id; ?>" class="btn btn-success"><i class="icon-upload-alt"></i> Estado</a></td>
		</tr>
